"""repositories/crud.py - 兼容层 (旧 crud.xxx 调用转发到 models)

旧的 `from backend.repositories.crud import crud` / `from backend.repositories.crud.db import ...` 仍然能工作，
但所有实现都基于 SQLAlchemy ORM (在 backend/repositories/models.py)。
"""
# 别名以兼容旧代码: from backend.repositories.crud import crud; crud.list_symbols() 仍然可用
...
class _CrudModule:
    ...
    def __getattr__(self, name):
        import backend.repositories.models as m
