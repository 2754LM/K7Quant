...
from backend.repositories.binance_data import get_kline, get_many
from backend.core.helpers import df_dates, to_records


def load_custom_factors_from_db():
    """启动时从 DB 把用户自定义因子加载到 FACTOR_REGISTRY"""
    try:
        from backend.repositories.models import list_factors as _db_list
        rows = _db_list()
        loaded = 0
        for r in rows:
...
def create_custom_factor(data: dict) -> dict:
    ...
    from backend.repositories.models import create_custom_factor as _db_create
...
def delete_custom_factor(factor_id: str):
    """删除自定义因子"""
    from backend.repositories.models import delete_custom_factor as _db_del
    _db_del(factor_id)
